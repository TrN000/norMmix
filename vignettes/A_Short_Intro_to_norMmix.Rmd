...
if(FALSE) ### FIXME
plot(ret) # gives error !!
if(FALSE) ### FIXME
ret # print(ret)  # also gives an error !
```

Use `?norMmixMLE` for details and see the section below.
...
0. for a single value `l`, norMmix() assumes all matrices to be diagonal with entries `l`
