...
\[
f({\bf x}) = \sum_{k=1}^{K} \pi_k \phi({\bf x};\mu_k, \Sigma_k), \hspace*{2em} and
F({\bf x}) = \sum_{k=1}^{K} \pi_k \Phi({\bf x};\mu_k, \Sigma_k),
\]

\[
 \text{ with } \pi_k \ge 0, \sum \pi_k = 1, \quad \phi(.) and \Phi(\cdot; \mu_k, \Sigma_k)
 \text{ the density and distribution function of a multivariate normal (aka *Gaussian*) distribution with mean }
 \mu_k  \text{ and covariance matrix } \Sigma_k,
\]
