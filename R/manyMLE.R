### model selection functionality for normal mixtures
...
manyMLE <- function(x, k, models=1:10, 
    ...
    class(ret) <- "manynorMmixMLE"

    if (!is.null(savdir)) {
        savename <- ifelse(is.null(name), 
                           sprintf("fit_dim=%d_n=%d.rds", p, n),
                           paste0(name, ".rds")) ## here maybe include dim, n etc?
        saveRDS(list(fit=ret), file=file.path(normalizePath(savdir),savename))
    }
    ret
}

nobs.manynorMmixMLE <- function(object, ...) {object$n}

logLik.manynorMmixMLE <- function(object, ...) {
    stopifnot(inherits(object, "manynorMmixMLE"))
    k <- object$k
...
npar.manynorMmixMLE <- function(object, ...) {
    k <- object$k
    p <- object$p
    models <- object$models
...
BIC.manynorMmixMLE <- function(object, ...) {
    n <- object$n
    k <- object$k
    models <- object$models
    npar <- npar(object)
...
AIC.manynorMmixMLE <- function(object, ..., k = 2) {
    models <- object$models
    npar <- npar(object)
    ll <- logLik(object)
    val <- npar*k - 2*ll
...
print.manynorMmixMLE <- function(x, ...) {
    n    <- nobs(x)
    co   <- x$k
    mo   <- x$models
    dim  <- x$p
...
displayError <- function(obj) {
    stopifnot(inherits(obj, "manynorMmixMLE"))
    k      <- obj$k
    models <- obj$models
...
cond <- function(obj) {
    stopifnot(inherits(obj, "manynorMmixMLE")) # returns matrix of nobs/npar
    k <- obj$k
    models <- obj$models
    val <- matrix(0, length(k), length(models))
    rownames(val) <- k
